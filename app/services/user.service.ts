...
export class UserService {
  ...
  getAllUsers(): Observable<UserReadDto[]> {
    return this.client.get<UserReadDto[]>("https://localhost:7012/GetAll");
  }
  getAdmins(): Observable<UserReadDto[]> {
    return this.client.get<UserReadDto[]>("https://localhost:7012/GetAllAdmins");
  }
  ...
  RegisterAdmin(UserRegister: UserAddDTO): Observable<any> {
    return this.client.post<string>(`https://localhost:7012/Admin/Register`, UserRegister);
  }
  IsLoggedIn(): Boolean {
    var logged = false;
  ...
  DeleteUser(userId: string) {
    return this.client.delete(`https://localhost:7012/Remove?id=${userId}`);
  }
  ...
  getClaims() {
    const requestOptions = { headers: this.RequestOption() };
    const string = "Call"
    return this.client.post(`https://localhost:7012/Claims`, string, requestOptions);
  }
  CheackClaims() {
    if (this.IsLoggedIn()) {
